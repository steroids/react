...
- в коде все абсолютные ссылки на саму себя (`import ... from '@steroidsjs/core/...') заменены на относительные
- упрощены конфиги `.eslintrc` и `tsconfig.json`
- линтер теперь проверяет и код тестов в том числе

## v3.1.8 (16.08.2026)

### Added

- Добавлены тесты валидации `NumberField` для обязательных и необязательных полей, граничных, дробных, отрицательных, форматированных и нечисловых значений

### Fixed

- Исправлена валидация `NumberField`: необязательные `min` и `max` теперь проверяются независимо и только когда указаны
- Непустое значение `NumberField` теперь проверяется независимо от `required`: учитываются числовой формат, ограничение `decimal`, флаг `isCanBeNegative` и разделитель тысяч `thousandSeparator`

## v3.1.7 (05.08.2026)

### Added

- Добавлена документация для `PhoneField`

## v3.1.6 (05.08.2026)

### Added

- Добавлен компонент `PhoneField` с выбором страны и форматированием номера по соответствующей телефонной маске
- В типизацию компонентов `InputField` и `NumberField` добавлены коллбеки `onFocus` и `onBlur`

### Fixed

- Исправлена конфигурация `exports` в `package.json` для импорта подмодулей пакета
- Удалено неиспользуемое упоминание переменной окружения `APP_BACKEND_URL` в `HttpComponent`

## v3.1.5 (26.06.2026)

### Added

- Добавлена логика промежуточного состояния `indeterminate` для компонентов `CheckboxField`, `CheckboxTreeField`. Происходит глубокая проверка по дочерним элементам родителя, если не все потомки активны, то родительский чекбокс приобретает состояние `indeterminate`
...
- Добавлен флаг `isCritical` в конфиг `useFetch` для ssr. Если запрос критический для страницы — ssr вернет страницу со статус кодом ошибки.

## v3.0.92 (05.02.2026)

### Added

- Добавлена возможность передать `itemsKey` и `action` в компонент `TreeTable`

### Fixed

- Исправлено изолирование авторизационных куки для поддоменов
...
- Добавлены `viewProps` для `Modal`

## v3.0.89 (26.01.2026)

### Added

- Добавлена возможность указывать, что высота поля должна быть равна высоте контента с помощью флага autoHeight в компоненте "TextField"
- Добавлен `InfiniteScroll` в компонент `List`
- Добавлено изолирование авторизационных куки для поддоменов. Для использования куки для поддоменов добавлен флаг `shareBetweenSubdomains` в `ClientStorageComponent`

### Fixed

- Убрали из списка зависимостей `query-string`, теперь используется `qs`
- Исправлен баг с поиском и обновлением параметров при запросе в `DropDownField`

## v3.0.88 (20.01.2026)

### Fixed

- Исправлен баг с InfiniteScroll в компоненте `Grid`

## v3.0.87 (15.01.2026)

### Fixed

- Исправлен порядок валидации полей в компоненте `Form`

## v3.0.86 (15.01.2026)

### Fixed

- Исправлен баг, из-за которого в SSR в компоненте `List` вместо списка отображался `Loader`

## v3.0.85 (14.01.2026)

### Added

- Добавлена возможность передавать сторонние валидаторы форм через проп validator

### Fixed

- Исправлен баг, при котором `EmailField` не реагировал на `required={false}`
...
- Добавлена возможность указывать временной диапазон для `TimeField` и диапазон дат для `DateField`
...
- Добавлен required для `NumberField`
...
- Добавлены `viewProps` для `Nav`

## v3.0.78 (02.12.2025)

### Added

- Добавлена возможность передать drop aa для FileField

## v3.0.77 (01.12.2025)

### Added

- Добавлена возможность указать аттрибут accept для FileField

## v3.0.76 (24.11.2025)

### Added

- Добавлены `viewProps` для `Menu`
...
- Исправлен баг, из-за которого `AutoCompleteField` выпадающий список не закрывался автоматически после выбора элемента
...
- Добавлена возможность пробрасывать `viewProps` в `Grid` компонент
