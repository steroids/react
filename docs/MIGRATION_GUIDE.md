...
## Redux 5 / React-Redux 9 upgrade

`redux` поднят с v4 до v5, `react-redux` — с v8 до v9. Если ваш проект работает со стором только через хуки/компоненты `@steroidsjs/core`, никаких действий не требуется.

### 1. Пакеты

Если проект напрямую зависит от `redux`/`react-redux` (пишет свои редьюсеры/селекторы поверх
того же стора, что и `@steroidsjs/core`) — поднять их до `redux@^5`/`react-redux@^9` вместе с
`@steroidsjs/core`. Иначе в дереве зависимостей может оказаться два экземпляра `react-redux` с
разными `ReactReduxContext`, и `useSelector` в части компонентов будет молча читать не тот
стор.

### 2. Тип `AnyAction`

Если проект импортирует `AnyAction` из `redux` — начиная с redux 5 он деприкейтед в пользу
`UnknownAction`, но остаётся экспортируемым и рабочим. Миграция не обязательна.
