...
const routesCache = {};
const findRecursive = (items, checkHandler, pathItems = null) => {
    let finedItem = null;
    (items || []).forEach(item => {
...
const findRecursivePage = (items, pageId, pathItems = null) => {
    return findRecursive(items, item => item.id === pageId, pathItems);
};
const checkActiveRecursive = (pathname, item) => {
    ...
    if (!match) {
        return !!(item.items || []).find(sub =>
            checkActiveRecursive(pathname, sub)
        );
    }
    return true;
};
...
export const getBreadcrumbs = (state, pageId = null, params = {}) => {
    ...
    return items
        ...
        .map(route => buildNavItem(state, route, params));
};
...
export const getParentRoute = (state, level = 1) => {
    ...
    const breadcrumbs =
        (currentRoute && getBreadcrumbs(state, currentRoute.id)) || [];
...
export const getCurrentRoute = state => {
    ...
    findRecursive([root], item => {
        ...
        if (finedRoute) {
            if (
                !routesCache[item.id] ||
                !_isEqual(routesCache[item.id], finedRoute)
            ) {
...
export const getNavItems = (state, parentPageId = null, params = {}) => {
    ...
    return route
        ? (route.items || [])
            .filter(item => item.isVisible !== false)
            .map(item => buildNavItem(state, item, params))
        : [];
};
